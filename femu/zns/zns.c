...
static void zns_init_zoned_state(NvmeNamespace *ns)
{
    ...
    // for pure QLC ZNS SSDs, all zones are configured in QLC
    int mode = (ns->ctrl->hzns_mode == 0? 1: 0);
...
{
    uint16_t status;
    if (ns->ctrl->hzns_mode == 2) {
      NvmeCmd *cmd = (NvmeCmd *)&req->cmd;
      zone->mode = cmd->cdw13 >> 9;
      femu_debug("open zone %d mode %s\n", zone->pos, zone->mode? "QLC" : "SLC");
    }
                                ...
                                NvmeZoneState state, NvmeRequest *req)
{
    ...
    case NVME_ZONE_STATE_EMPTY:
        ...
        // zone finish leads to unused space
        if (ns->ctrl->hzns_mode == 1) {
            struct zonessd *ssd = ns->ctrl->znsssd;
            uint64_t unused_space = zone->w_ptr - zone->d.wp;
            if (zone->mode == 0) {
                ssd->used_slc_cap += unused_space;
                ssd->used_cap += unused_space << 2;
            } else {
                ssd->used_qlc_cap += unused_space;
                ssd->used_cap += unused_space;
            }
            femu_debug("finish zone %u unused %lu\n", zone->pos, unused_space);
        }     
        zone->d.wp = zone->w_ptr;
        zns_assign_zone_state(ns, zone, NVME_ZONE_STATE_FULL);
        /* fall through */
                               ...
                               NvmeZoneState state, NvmeRequest *req)
{
    ...
    if (ns->ctrl->hzns_mode == 1) {
        ...
        struct zonessd* ssd = ns->ctrl->znsssd;
        uint64_t recycled_space = zone->d.wp - zone->d.zslba;
        if (zone->mode == 0) {
            ssd->used_slc_cap -= recycled_space;
            ssd->used_cap -= recycled_space << 2;
        } else {
            ssd->used_qlc_cap -= recycled_space;
            ssd->used_cap -= recycled_space;
        }
        femu_debug("recycle %lu (wp %lu slba %lu), used_cap %lu, used_slc_cap %lu, used_qlc_cap %lu\n", 
                  recycled_space, zone->d.wp, zone->d.zslba, 
                  ssd->used_cap, ssd->used_slc_cap, ssd->used_qlc_cap);
    }
    zonessd_reset(ns->ctrl, zone->pos, req, zone->mode);
    zns_aio_zone_reset_cb(req, zone);
    if (ns->ctrl->hzns_mode != 0) {
      zone->mode = 0; // convert back to slc
    }
    zone->accessed = 0;
        ...
        if (proc_mask & NVME_PROC_CLOSED_ZONES) {
            QTAILQ_FOREACH_SAFE(zone, &n->closed_zones, entry, next) {
                status = zns_bulk_proc_zone(ns, zone, proc_mask, op_hndlr,
                                             req);
                maxlat = maxlat > req->reqlat ? maxlat : req->reqlat;
...
static void zns_clear_accessed_bits(FemuCtrl *n)
{
    for (int i = 0; i < n->num_zones; i++) {
        n->zone_array[i].accessed = 0;
    }
    femu_debug("clear accessed bits\n");
}

static int zns_select_a_zone(FemuCtrl *n)
{
    NvmeZone *zone = NULL;
    uint32_t accessed = 0, not_accessed = 0;
    for (int i = 0; i < n->num_zones; i++) {
        zone = &n->zone_array[i];
        if (!zone->mode) {
            if (zone->accessed) accessed ++;
            else not_accessed ++;
        }
    }
    femu_debug("in zns_select_a_zone, #accessed/#not_accessed SLC zone %u/%u\n", accessed, not_accessed);
    for (int i = 0; i < n->num_zones; i++) {
        zone = &n->zone_array[i];
        if (!zone->mode && !zone->accessed) {
            return i;
        }
    }
    for (int i = 0; i < n->num_zones; i++) {
                         ...
                         NvmeRequest *req, int *mode)
{
    ...
    *mode = zone->mode;
    if (n->hzns_mode == 1) {
        zone->accessed = 1;
        if (++ns->tt_accessed > 10000) { // TO FIX
            zns_clear_accessed_bits(n);
            ns->tt_accessed = 0;
        }
    }
    femu_debug("read zone %d slba %lu nlb %u\n", zone->pos, slba, nlb);
                          ...
                          NvmeRequest *req, int *mode)
{
    ...
    status = nvme_check_mdts(n, data_size);
    if (status) {
        femu_err("zns check mdts failed\n");
        goto err;
    }

    status = zns_check_bounds(ns, slba, nlb);
    if (status) {
        femu_err("zns check bounds failed\n");
        goto err;
    }
    ...
    if (status) {
        femu_err("zns check zone write failed and status is %hu\n", status);
        goto err;
    }

    status = zns_auto_open_zone(ns, zone);
    if (status) {
        femu_err("zns open zone failed and status is %hu\n", status);
        goto err;
    }
    ...
    *mode = zone->mode;
    ...
    if (zone->mode == 0) {
        ssd->used_slc_cap += nlb;
        ssd->used_cap += nlb << 2;
    } else {
        ssd->used_qlc_cap += nlb;
        ssd->used_cap += nlb;
        ns->tt_qlc_writes += nlb;
    }
    femu_log("total_writes %lu qlc_writes %lu qlc_flush %lu\n", ns->tt_writes, ns->tt_qlc_writes, ns->tt_qlc_flush);

    if (n->hzns_mode == 1) {
        zone->accessed = 1;
        if (++ns->tt_accessed > 100000) {
            zns_clear_accessed_bits(n);
            ns->tt_accessed = 0;
        }
        
        // trigger SLC->QLC when space is not enough
        int slc_cap_setups[11] = {56, 56, 50, 40, 30, 25, 20, 10, 10, 10, 10};
        int threshold = slc_cap_setups[ssd->used_cap * 10 / ssd->total_cap];
        femu_log("used_slc %lu used_qlc %lu used %lu used_ratio %lu used_slc_ratio %lu slc_threshold %d\n", 
                  ssd->used_slc_cap, ssd->used_qlc_cap, ssd->used_cap, ssd->used_cap * 100 / ssd->total_cap,
                  ssd->used_slc_cap * 100 * 4 / ssd->total_cap, threshold);

        while (ssd->used_slc_cap * 100 * 4 / ssd->total_cap > threshold) {
            // migrate zones based on accessed time until space becomes enough
            uint32_t zone_idx = zns_select_a_zone(n);
            NvmeZone* mzone = &n->zone_array[zone_idx];
            assert(!mzone->mode);
            femu_debug("ready to migrate zone %u, lba %lu - %lu\n", zone_idx, mzone->d.zslba, mzone->w_ptr);
            zonessd_migrate(ssd, mzone->d.zslba, mzone->w_ptr);
            mzone->mode = 1; // to qlc
            uint64_t recycled_space = mzone->w_ptr - mzone->d.zslba;
            ssd->used_slc_cap -= recycled_space;
            ssd->used_qlc_cap += recycled_space;
            ssd->used_cap -= recycled_space * 3;
            ns->tt_writes += recycled_space;
            ns->tt_qlc_writes += recycled_space;
            ns->tt_qlc_flush += recycled_space;
            femu_debug("convert zone %u (data %lu state %d) to QLC, used_ratio %lu used_slc_ratio %lu\n", 
                      zone_idx, recycled_space, zns_get_zone_state(zone),
                      ssd->used_cap * 100 / ssd->total_cap,
                      ssd->used_slc_cap * 100 * 4 / ssd->total_cap);
            femu_log("total_writes %lu qlc_writes %lu qlc_flush %lu\n", ns->tt_writes, ns->tt_qlc_writes, ns->tt_qlc_flush);
        }
    }
    femu_debug("write zone %d slba %lu nlb %u\n", zone->pos, slba, nlb);
...
static uint16_t zns_copy(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                          NvmeRequest *req, int *mode)
{
    NvmeRwCmd *rw = (NvmeRwCmd *)cmd;
    uint64_t slba = le64_to_cpu(rw->slba);
    ...
    uint32_t src = cmd->cdw14, nlb = cmd->cdw15;
    femu_debug("Copy: zone %d wp %lu dst %lu src %u nlb %u\n", zone->pos, zone->w_ptr, slba, src, nlb);
    ...
    *mode = zone->mode;
    ...
    // copy source data to the destination
    uint64_t dst_offset = zns_l2b(ns, slba);
    uint64_t src_offset = zns_l2b(ns, src);
    uint64_t nbytes = zns_l2b(ns, nlb);
    backend_copy(n->mbe, dst_offset, src_offset, nbytes);
    femu_debug("dst offset %lu src offset %lu nbytes %lu\n", dst_offset, src_offset, nbytes);
...
static uint16_t zns_io_cmd(FemuCtrl *n, NvmeNamespace *ns, NvmeCmd *cmd,
                           NvmeRequest *req)
{
    int opcode = cmd->opcode, mode = 0;
    uint16_t res;

    switch (opcode) {
    case NVME_CMD_READ:
        res = zns_read(n, ns, cmd, req, &mode);
        zonessd_read(n->znsssd, req, mode);
        return res;
    case NVME_CMD_WRITE:
        res = zns_write(n, ns, cmd, req, &mode);
        zonessd_write(n->znsssd, req, mode);
        return res;
    case NVME_CMD_COPY:
        res = zns_copy(n, ns, cmd, req, &mode);
        zonessd_copy(n->znsssd, req, mode);
        return res;
    ...
    case NVME_CMD_ZONE_APPEND:
        zonessd_append(n->znsssd, req, mode);
...
{
    NvmeNamespace *ns = &n->namespaces[0];
    n->znsssd = g_malloc0(sizeof(struct zonessd));
    zns_set_ctrl(n);
    ...
    zonessd_init(n);
    ns->tt_accessed = ns->tt_writes = ns->tt_qlc_writes = ns->tt_qlc_flush = 0;
}
