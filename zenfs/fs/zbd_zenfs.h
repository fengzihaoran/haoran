...
class Zone {
 ...
 public:
  ...
  bool mode_; // SLC: 0, QLC: 1
  uint64_t available_capacity_; // SLC: 1/4 max_capacity, QLC: max_capacity_

  IOStatus Reset();
  IOStatus Finish();
  IOStatus ExplicitOpen(bool mode);
  IOStatus Close();
...
class ZonedBlockDeviceBackend {
 ...
 public:
  ...
  virtual IOStatus ExplicitOpen(uint64_t start, bool mode) = 0;
  virtual IOStatus Copy(uint64_t dst, uint64_t src, uint64_t len) = 0;
...
class ZonedBlockDevice {
 public:
  ...
  bool space_shortage_ = false;
  std::atomic<long> active_qlc_io_zones_;
  std::mutex active_qlc_zone_mtx_;
 ...
 private:
  ...
  uint32_t slc_cap_setups_[11] = {56, 56, 50, 40, 30, 25, 20, 10, 10, 10, 10};
 ...
 public:
  ...
  bool IsSpaceShortage() { return space_shortage_; }
  uint64_t GetUsedRatio();
  uint64_t GetSLCWatermark(uint64_t used) { return slc_cap_setups_[used / 10]; }
  ...
  void LogZoneStats();
  void LogHybridZoneStats(Zone *zone);
  void LogZoneUsage();
  void LogGarbageInfo();
 ...
 private:
  ...
  IOStatus GetBestOpenZoneMatch(Env::WriteLifeTimeHint file_lifetime,
                                ...
                                uint32_t min_capacity = 0, bool qlc = false);
