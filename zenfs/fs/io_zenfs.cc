...
IOStatus ZoneFile::PositionedRead(uint64_t offset, size_t n, Slice* result,
  ...
  // printf("GetExtent start %lu len %lu\n", extent->start_, extent->length_);
...
IOStatus ZoneFile::AllocateNewZone() {
  // printf("in ZoneFile::AllocateNewZone\n");
  Zone* zone;
...
IOStatus ZoneFile::MigrateData(uint64_t offset, uint32_t length,
  ...
  if (simple_copy) {
    zbd_->Copy(target_zone->wp_, offset, length);
    // printf("after copy, wp %lu cap %lu\n", target_zone->wp_, target_zone->capacity_);

    int block_sz = zbd_->GetBlockSize();
    int pad_sz = (length % block_sz != 0? (block_sz - length % block_sz) : 0);
    target_zone->wp_ += length + pad_sz;
    target_zone->capacity_ -= length + pad_sz;

    zbd_->AddBytesWritten(length + pad_sz);
  } else {
    ...
    while (length > 0) {
      ...
      length -= read_sz;
      offset += r;
      // printf("read_sz %u pad_sz %d r %d\n", read_sz, pad_sz, r);
    }
    // printf("after migration, offset delta %lu length %u pad_sz %d\n",
    //        offset - prev_offset, length, pad_sz);
