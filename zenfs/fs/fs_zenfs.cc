...
void ZenFS::GCWorker() {
  while (run_gc_worker_) {
    ...
    uint64_t r = zbd_->GetUsedRatio();
    uint64_t slc_r = r >> 32, total_r = r & 0xffffffff;
    uint64_t slc_threshold = zbd_->GetSLCWatermark(total_r);
    // printf("in GCWorker, slc_r %lu%% total_r %lu%% watermark %lu%%\n", slc_r, total_r, slc_threshold);

    if (slc_r <= slc_threshold) {
      zbd_->space_shortage_ = false;
      zbd_->LogHybridZoneStats(nullptr);
      continue;
    }
    zbd_->space_shortage_ = true;
    ...
    // three reclaimation groups now
    // correspond to [0, 50%), [50%, 80%], (80%, 100%]
    int group_n = 3, group_id;
    std::vector<std::unordered_map<uint64_t, uint64_t> > rg(group_n);
    uint64_t gc_approx;
    for (const auto& zone : snapshot.zones_) {
      if ((!zone.used_capacity) || (zone.max_capacity > 0x10000000)) // TO FIX
        continue;
      if (zone.lifetime < 3) // only consider zones with SST
        continue;

      gc_approx = 100 - 100 * zone.used_capacity / zone.max_capacity; // actually available_capacity
      if (gc_approx < 50) group_id = 0;
      else if (gc_approx > 80) group_id = 2;
      else group_id = 1;

      rg[group_id][zone.start] = (gc_approx == 100? 1 : (100 - gc_approx));
      rg[group_id][zone.start] |= (zone.lifetime << 16);
      rg[group_id][zone.start] |= (zone.max_capacity << 32);
      // printf("add zone start %lu mc %lu lt %d gc %lu\n", zone.start, zone.max_capacity, zone.lifetime, gc_approx);
    }
    // printf("#zones in each group [%lu, %lu, %lu]\n", rg[0].size(), rg[1].size(), rg[2].size());

    // When the SLC used ratio is larger than the threshold, we migrate extents in SLC to QLC.
    // We priorize SLC zones with: (1) lowest level -> largest lifetime and minimum reads
    // (2) highest reclaimable space -> more space can be reclaimed then reused
    uint64_t approx_reclaimed = 0;
    uint64_t expected_reclaimed = zbd_->GetZoneSize() / 100 * (slc_r - slc_threshold) * zbd_->io_zones.size();

    bool extent_base = false;
    std::vector<ZoneExtentSnapshot*> migrate_exts;

    if (extent_base) {
      int amp_factor;
      while (approx_reclaimed < expected_reclaimed) {
        // loop from the lowest level
        for (int level = 5; level >= 3; level --) {
          // loop from the highest reclaimable group
          for (int gid = 2; gid >= 0; gid --) {
            for (auto& ext : snapshot.extents_) {
              if (ext.lifetime != level) continue;
              // printf("-- ext start %lu len %lu lt %d\n", ext.start, ext.length, ext.lifetime);
              if (rg[gid].find(ext.zone_start) != rg[gid].end()) {
                migrate_exts.push_back(&ext);
                amp_factor = 100 / (rg[gid][ext.zone_start] & 0xffff);
                amp_factor = amp_factor > 5? 5 : amp_factor; // TO FIX
                // we need amp_factor because we reclaim space in extents, not zones
                approx_reclaimed += (ext.length << 2) * amp_factor;
                if (approx_reclaimed >= expected_reclaimed)
                  goto out;
              }
            }
          }
        }
        printf("no more extents to reclaim, approx %lu expected %lu\n", 
              approx_reclaimed, expected_reclaimed);
        exit(-1);
      }
      ...
      while (approx_reclaimed < expected_reclaimed) {
        ...
        for (int level = 5; level >= 3; level --) {
          // loop from the highest reclaimable group
          for (int gid = 2; gid >= 0; gid --) {
            // loop zones in each group
            for (auto it = rg[gid].begin(); it != rg[gid].end(); it++) {
              uint64_t zone_start = it->first;
              int zone_lifetime = (it->second >> 16) & 0xffff;
              // printf("check zone start %lu lt %d\n", zone_start, zone_lifetime);
              if (zone_lifetime != level) continue;
              for (auto& ext : snapshot.extents_) {
                if (ext.zone_start != zone_start) continue;
                migrate_exts.push_back(&ext);
              }
              // printf("approx_reclaimed %lu -> %lu\n", approx_reclaimed, approx_reclaimed + ((it->second >> 32) << 2));
              approx_reclaimed += (it->second >> 32) << 2;
              if (approx_reclaimed >= expected_reclaimed)
                goto out;
            }
          }
...
out:
    // printf("migrate %lu/%lu extents, approx %lu expected %lu\n", migrate_exts.size(),
    //        snapshot.extents_.size(), approx_reclaimed, expected_reclaimed);

    if (migrate_exts.size() > 0) {
      IOStatus s;
      s = MigrateExtents(migrate_exts);
      if (!s.ok()) {
        printf("migrate extents failed!\n");
      }
    }
    // zbd_->LogHybridZoneStats(nullptr);
  }
  ...
  if (options.zone_file_) {
    ...
    for (const auto& file_it : files_) {
      ...
      for (auto* ext : file.GetExtents()) {
        snapshot.extents_.emplace_back(*ext, file.GetFilename(), file.GetWriteLifeTimeHint());
      }
...
IOStatus ZenFS::MigrateFileExtents(
  ...
  for (ZoneExtent* ext : new_extent_list) {
    ...
    // Allocate a new migration zone (for HZNS, the zone must be in QLC)
    ...
    if (target_zone == nullptr) {
      ...
      // printf("Fail to allocate a QLC zone for migration\n");
      continue;
    }

    uint64_t target_start = target_zone->wp_;
    // printf("extent start %lu length %lu from zone %lu to %lu\n", ext->start_, ext->length_,
    //        ext->zone_->GetZoneNr(), target_zone->GetZoneNr());
    if (zfile->IsSparse()) {
      ...
      // printf("%s: ext start %lu length %lu wp %lu\n", fname.c_str(), ext->start_, ext->length_, target_start);
    ...
    // printf("migrate to zone %lu start %lu cap %lu ext len %lu\n", target_zone->GetZoneNr(),
    //        ext->start_, target_zone->GetCapacityLeft(), ext->length_);
