 ...
 public:
  ZoneSnapshot(const Zone& zone)
      : start(zone.start_),
        ...
        max_capacity(zone.available_capacity_), // store available_capacity in the snapshot
...
class ZoneExtentSnapshot {
 ...
 public:
  ZoneExtentSnapshot(const ZoneExtent& extent, const std::string fname, Env::WriteLifeTimeHint hint)
      : start(extent.start_),
 ...
 public:
  ZoneFileSnapshot(ZoneFile& file)
    ...
    for (const auto* extent : file.GetExtents()) {
      extents.emplace_back(*extent, filename, file.GetWriteLifeTimeHint());
    }
  }
};
